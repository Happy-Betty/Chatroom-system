...
public class Server {
	...
	private synchronized boolean broadcast(String message) {
		...
		int groupN = -1;
                                    ...
                                    if(check.equals(tocheck))
                                    {
                                        ...
                                        if(p == false) {
                                            for(int yy=al.size(); --yy>=0;)
                                            {
                                                    ...
                                                    {
                                                        ct2.writeMsg("Receiver but you are not on the same group");
                                                        break;
                                                    }
                                            }
                                            return true;
			...
			private synchronized void broadcastToGroup(int groupIndex, String message) {
				String time = sdf.format(new Date());
				String messageLf = time + " " + message + "\n";
			
				for (String curMember : groupMembers.get(groupIndex)) {
					for (ClientThread ct1 : al) {
						if (ct1.getUsername().equals(curMember)) {
							ct1.writeMsg(messageLf);
							break;
						}
					}
	...
	synchronized void remove(int id, int groupIndex ) {
		...
		for(int i = 0; i < al.size(); ++i) {
			...
			if(ct.id == id) {
				...
				al.remove(i);
				broadcastToGroup(groupIndex, notif + disconnectedClient + " has left the chat room." + groupNames.get(groupIndex)  + notif);
				break;
			}
		}	
}
                                    ...
                                    if(p == true) {
                                        ...
                                        for(int i=0; i < groupNum; i++) {
                                            ...
                                            if(message.equals(curName))
                                            {
												...
												if (groupMembers.get(i).size() == 1) {
													// First user joining the group
													writeMsg("You have joined group " + groupNames.get(gr));
													
												}else { 
													writeMsg("You have joined group " + groupNames.get(gr));

													// Send broadcast message to existing group members
												String timing = sdf.format(new Date());
												String joinNotification = timing + notif + username + " has joined the group " + groupNames.get(gr) + notif;
												for (String member : groupMembers.get(i)) {
													if (!member.equals(username)) {
														for (ClientThread ct1 : al) {
															if (ct1.getUsername().equals(member)) {
																ct1.writeMsg(joinNotification);
																break;
															}
														}
													}
                                    ...
                                    if(gr == -1) {
                                        ...
                                        for(int j=0; j<groupMembers.get(gr).size(); j++) {
                                            ...
                                            if(curMember.equals(username)) {
                                                ...
                                                p = true;
												writeMsg("You left group " + groupNames.get(gr));
                                                break;
                                            }
                                        }

										// Send notification to the remaining members of the group
										if (p) {
											// String notification = notif + username + " has left the group " + groupNames.get(gr) + notif;
											broadcastToGroup(gr,notif + username + " has left group " + groupNames.get(gr) + notif);
										}
